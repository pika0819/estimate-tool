...
class EstimatePDFGenerator:
    def __init__(self, df: pd.DataFrame, params: Dict[str, str]):
        ...
        self._setup_columns()
        self.total_estimated = df['見積金額'].apply(parse_amount).sum() # 税抜合計
    ...
    def draw_breakdown_table(self):
        ...
        for l1, l2_dict in data_struct.items():
            ...
            valid_l2_count = sum(1 for l2 in l2_dict if l2)
            rows_needed = 1 + valid_l2_count + 1 
            
            if self._check_space(rows_needed, TITLE): pass
    ...
    def draw_detail_pages(self, p_num):
        ...
        for l1 in seen_l1:
            ...
            for l2, items in l2_dict.items():
                ...
                current_l3 = ""; current_l4 = ""; sub_l3 = 0; sub_l4 = 0
            ...
            if l1 == "諸経費":
                ...
                for lbl, val in labels:
                    ...
                    self.c.setFont(self.font, 11)
                    self.c.setFillColor(Style.COLOR_TOTAL)
                    self.c.drawRightString(self.col_x['amt'] + self.col_widths['amt'] - 2*mm, self.current_y-5*mm, f"{int(val):,}")
                    self.current_y -= Style.ROW_HEIGHT

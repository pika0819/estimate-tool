...
# Streamlit環境の物理的制約: GitHub上の同じ階層にあるフォントファイルを直接読み込む
FONT_FILE = "NotoSerifJP-Regular.ttf" # ※GitHubにアップロードしたファイル名と一致させること
CUSTOM_FONT = "NotoSerifJP"
pdfmetrics.registerFont(TTFont(CUSTOM_FONT, FONT_FILE))

# --- 以下、Colabで作成した parse_amount や class EstimatePDFGenerator のコードをそのまま貼り付け ---

# Step 2: PDF生成エンジンの定義処理（表紙なし版） #
...
def to_wareki(date_str: str) -> str:
    try:
        ...
        if y >= 2019:
            r_y = y - 2018
            return f"令和 {'元年' if r_y == 1 else str(r_y)+'年'} {m}月 {d}日"
        return dt_obj.strftime("%Y年 %m月 %d日")
    except: return str(date_str)

class Style:
    COLOR_L1 = colors.HexColor('#0D5940'); COLOR_L2 = colors.HexColor('#1A2673')
    COLOR_L3 = colors.HexColor('#994D1A'); COLOR_TEXT = colors.HexColor('#000000')
    COLOR_TOTAL = colors.HexColor('#B31A26'); COLOR_ACCENT_BLUE = colors.HexColor('#26408C')
    MARGIN_TOP = 35 * mm; MARGIN_BOTTOM = 21 * mm; ROW_HEIGHT = 7 * mm
    HEADER_HEIGHT = 9 * mm; X_BASE = 15 * mm; INDENT_L1 = 1.0 * mm
    INDENT_L2 = 2.5 * mm; INDENT_L3 = 4.5 * mm; INDENT_ITEM = 6.0 * mm
...
class EstimatePDFGenerator:
    def __init__(self, df: pd.DataFrame, params: dict):
        ...
        # ダブルカウント防止処理
        self.valid_df = self.df[(self.df['大項目'].astype(str).str.strip() != '') & (self.df['名称'].astype(str).str.strip() != '')]

        self.content_width = self.width - 30 * mm
        self._setup_columns()
        self.total_grand = self.valid_df['見積金額'].apply(parse_amount).sum()
    ...
    def _setup_columns(self):
        ...
        for k, w in widths.items():
            self.col_x[k] = curr_x; curr_x += w
        self.col_widths = widths; self.right_edge = curr_x
        self.y_start = self.height - Style.MARGIN_TOP

    def _draw_bold_string(self, x, y, text, size, color=colors.black):
        self.c.saveState(); self.c.setLineWidth(size * 0.03)
        t_obj = self.c.beginText(x, y); t_obj.setFont(self.font, size)
        t_obj.setFillColor(color); t_obj.setStrokeColor(color); t_obj.setTextRenderMode(2)
        t_obj.textOut(str(text)); self.c.drawText(t_obj); self.c.restoreState()
    ...
    def _draw_grid(self, y_top, y_bottom):
        self.c.saveState(); self.c.setLineWidth(0.5); self.c.setStrokeColor(colors.grey)
        for k in self.col_x: self.c.line(self.col_x[k], y_top, self.col_x[k], y_bottom)
        self.c.line(self.right_edge, y_top, self.right_edge, y_bottom)
        curr = y_top
        while curr > y_bottom - 0.1:
            self.c.setStrokeColor(colors.black); self.c.line(Style.X_BASE, curr, self.right_edge, curr)
            curr -= Style.ROW_HEIGHT
        self.c.setStrokeColor(colors.black); self.c.line(Style.X_BASE, y_bottom, self.right_edge, y_bottom)
        self.c.restoreState()

    def _draw_page_header(self, p_num, title):
        hy = self.height - 20 * mm
        self.c.setFillColor(colors.black); self.c.setFont(self.font, 16)
        tw = self.c.stringWidth(title, self.font, 16); self.c.drawCentredString(self.width/2, hy, title)
        self.c.setLineWidth(0.5); self.c.line(self.width/2 - tw/2 - 5*mm, hy - 2*mm, self.width/2 + tw/2 + 5*mm, hy - 2*mm)
        self.c.setFont(self.font, 10); self.c.drawRightString(self.right_edge, hy, self.params.get('company_name', ''))
        self.c.drawCentredString(self.width/2, 10*mm, f"- {p_num} -")
        grid_y = self.y_start; self.c.setFillColor(colors.Color(0.95, 0.95, 0.95))
        self.c.rect(Style.X_BASE, grid_y, self.right_edge - Style.X_BASE, Style.HEADER_HEIGHT, fill=1, stroke=0)
        self.c.setFillColor(colors.black); self.c.setFont(self.font, 10)
        labels = {'name':"名 称", 'spec':"規 格", 'qty':"数 量", 'unit':"単位", 'price':"単 価", 'amt':"金 額", 'rem':"備 考"}
        for k, txt in labels.items():
            self.c.drawCentredString(self.col_x[k] + self.col_widths[k]/2, grid_y + 2.5*mm, txt)
        self.c.setStrokeColor(colors.black); self.c.setLineWidth(0.5)
    ...
    def draw_summary(self):
        ...
        self.c.setFont(self.font, 20); self.c.drawString(40*mm, self.height - 50*mm, f"{self.params.get('client_name','')} ")
        ...
        line_sx = box_left + 10*mm; label_end_x = line_sx + 28*mm; colon_x = label_end_x + 1*mm; val_start_x = colon_x + 5*mm; line_ex = box_left + box_width - 10*mm
        ...
        self.c.setFont(self.font, 12); self.c.drawString(val_start_x + self.c.stringWidth(amt_s, self.font, 18) + 5*mm, curr_y, f"(別途消費税  ¥ {int(self.tax_amount):,})")
        self.c.setLineWidth(0.5); self.c.line(line_sx, curr_y-2*mm, line_ex, curr_y-2*mm)
        curr_y -= gap * 1.5
        items = [("工 事 名", self.params.get('project_name','')), ("工事場所", self.params.get('location','')),
                 ("工    期", self.params.get('term','')), ("そ の 他", "別紙内訳書による"), ("見積有効期限", self.params.get('expiry',''))]
        for label, val in items:
            ...
            self.c.line(line_sx, curr_y-2*mm, line_ex, curr_y-2*mm); curr_y -= gap
        x_co = box_left + box_width - 90*mm; y_co = box_btm + 10*mm
        self.c.setFont(self.font, 13); self.c.drawString(x_co, y_co + 15*mm, self.params.get('company_name',''))
        self.c.setFont(self.font, 11); self.c.drawString(x_co, y_co + 10*mm, f"代表取締役   {self.params.get('ceo','')}")
        self.c.setFont(self.font, 10); self.c.drawString(x_co, y_co + 5*mm, f"〒 {self.params.get('address','')}")
        self.c.drawString(x_co, y_co, f"TEL {self.params.get('phone','')}  FAX {self.params.get('fax','')}")
        self.c.setFont(self.font, 12); self.c.drawString(self.width - 80*mm, box_top + 5*mm, to_wareki(self.params.get('date','')))
        self.c.showPage()
    ...
    def draw_total_summary(self, p_num):
        ...
        y = self.y_start
        l1_summary = self.valid_df.groupby('大項目', sort=False)['見積金額'].apply(lambda x: x.apply(parse_amount).sum()).reset_index()
        for _, row in l1_summary.iterrows():
            ...
            y -= Style.ROW_HEIGHT
        footer_start_y = Style.MARGIN_BOTTOM + (3 * Style.ROW_HEIGHT); y = footer_start_y
        ...
        for lbl, val in labels:
            ...
            self.c.setFont(self.font, 11); self.c.setFillColor(Style.COLOR_TOTAL)
    ...
    def draw_breakdown_pages(self, p_num):
        raw_rows = self.valid_df.to_dict('records')
        breakdown = {}; seen_l1 = []; seen_l2_by_l1 = {}
        for row in raw_rows:
            l1 = str(row.get('大項目', '')).strip(); l2 = str(row.get('中項目', '')).strip()
            amt = parse_amount(row.get('見積金額', 0))
            if not l1: continue
            if l1 not in seen_l1: seen_l1.append(l1); seen_l2_by_l1[l1] = []
            ...
            breakdown[l1]['items'][l2] += amt; breakdown[l1]['total'] += amt

        self._draw_page_header(p_num, "内 訳 明 細 書 (集計)")
        self._draw_grid(self.y_start, Style.MARGIN_BOTTOM - Style.ROW_HEIGHT)
        y = self.y_start; is_first = True
        
        for l1 in seen_l1:
            data = breakdown[l1]; sorted_l2 = seen_l2_by_l1[l1]
            spacer = 1 if not is_first else 0; rows_needed = spacer + 1 + len(sorted_l2) + 1
            if rows_needed > int((y - Style.MARGIN_BOTTOM) / Style.ROW_HEIGHT):
                self.c.showPage(); p_num += 1; self._draw_page_header(p_num, "内 訳 明 細 書 (集計)")
                self._draw_grid(self.y_start, Style.MARGIN_BOTTOM - Style.ROW_HEIGHT); y = self.y_start; is_first = True; spacer = 0
            if spacer: y -= Style.ROW_HEIGHT
            self._draw_bold_string(self.col_x['name'] + Style.INDENT_L1, y-5*mm, f"■ {l1}", 10, Style.COLOR_L1); y -= Style.ROW_HEIGHT
            for l2 in sorted_l2:
                if not l2: continue 
                ...
                self.c.drawRightString(self.col_x['amt'] + self.col_widths['amt'] - 2*mm, y-5*mm, f"{int(data['items'][l2]):,}")
                y -= Style.ROW_HEIGHT
            ...
            y -= Style.ROW_HEIGHT; is_first = False
        self.c.showPage()
        return p_num + 1

    def draw_detail_pages(self, p_num):
        data_tree = {}; seen_l1 = []; seen_l2_by_l1 = {}
        for row in self.valid_df.to_dict('records'):
            l1 = str(row.get('大項目', '')).strip(); l2 = str(row.get('中項目', '')).strip()
            ...
            item = row.copy()
            item.update({'amt_val': parse_amount(row.get('見積金額', 0)), 'qty_val': parse_amount(row.get('数量', 0)), 'price_val': parse_amount(row.get('売単価', 0)), 'l3': l3, 'l4': l4})
        ...
        for l1 in seen_l1:
            l2_dict = data_tree.get(l1, {})
            if not l2_dict: continue
            l1_total = sum([sum([i['amt_val'] for i in items]) for items in l2_dict.values()]); sorted_l2 = seen_l2_by_l1[l1]
            if not is_first:
                if y <= Style.MARGIN_BOTTOM + Style.ROW_HEIGHT * 2:
                    self.c.showPage(); p_num += 1; self._draw_page_header(p_num, "内 訳 明 細 書 (詳細)")
                    self._draw_grid(self.y_start, Style.MARGIN_BOTTOM - Style.ROW_HEIGHT); y = self.y_start
                else: y -= Style.ROW_HEIGHT
            ...
            for i_l2, l2 in enumerate(sorted_l2):
                items = l2_dict.get(l2, []); l2_total = sum([i['amt_val'] for i in items]); rows_to_draw = []
                if l2: rows_to_draw.append({'type': 'header_l2', 'label': f"● {l2}"})
                curr_l3 = ""; curr_l4 = ""; sub_l3 = 0; sub_l4 = 0; item_rows = []
                for itm in items:
                    ...
                    sub_l3 += amt; sub_l4 += amt; item_rows.append({'type': 'item', 'data': itm})
                ...
                if l2: rows_to_draw.append({'type': 'footer_l2', 'label': f"【{l2} 計】", 'amt': l2_total})
                is_last_l2 = (i_l2 == len(sorted_l2) - 1)
                if is_last_l2: rows_to_draw.append({'type': 'footer_l1', 'label': f"【{l1} 計】", 'amt': l1_total})
                else: rows_to_draw.extend([{'type': 'empty'}, {'type': 'empty'}])
                ...
                for b in rows_to_draw:
                    itype = b['type']; force_stay = (itype == 'footer_l1')
                    if y - Style.ROW_HEIGHT < Style.MARGIN_BOTTOM - 0.1 and not force_stay:
                        self.c.showPage(); p_num += 1; self._draw_page_header(p_num, "内 訳 明 細 書 (詳細)")
                        self._draw_grid(self.y_start, Style.MARGIN_BOTTOM - Style.ROW_HEIGHT); y = self.y_start
                        self._draw_bold_string(self.col_x['name']+Style.INDENT_L1, y-5*mm, f"■ {l1} (続き)", 10, Style.COLOR_L1); y -= Style.ROW_HEIGHT
                        if l2_started and itype != 'footer_l1':
                            if l2: self._draw_bold_string(self.col_x['name']+Style.INDENT_L2, y-5*mm, f"● {l2} (続き)", 10, Style.COLOR_L2); y -= Style.ROW_HEIGHT
                        if cur_l3_lbl: self._draw_bold_string(self.col_x['name']+Style.INDENT_L3, y-5*mm, f"{cur_l3_lbl} (続き)", 10, Style.COLOR_L3); y -= Style.ROW_HEIGHT
                        if cur_l4_lbl: self._draw_bold_string(self.col_x['name']+Style.INDENT_ITEM, y-5*mm, f"{cur_l4_lbl} (続き)", 9, colors.black); y -= Style.ROW_HEIGHT
                    ...
                    if itype == 'header_l2': self._draw_bold_string(self.col_x['name']+Style.INDENT_L2, y-5*mm, b['label'], 10, Style.COLOR_L2); l2_started = True
                    elif itype == 'header_l3': self._draw_bold_string(self.col_x['name']+Style.INDENT_L3, y-5*mm, b['label'], 10, Style.COLOR_L3); cur_l3_lbl = b['label']
                    elif itype == 'header_l4': self._draw_bold_string(self.col_x['name']+Style.INDENT_ITEM, y-5*mm, b['label'], 9, colors.black); cur_l4_lbl = b['label']
                    elif itype == 'item':
                        ...
                        self.c.setFont(self.font, 9); self.c.drawRightString(self.col_x['amt']+self.col_widths['amt']-2*mm, y-5*mm, f"{int(b['amt']):,}"); cur_l4_lbl = None
                        ...
                        self.c.drawRightString(self.col_x['amt']+self.col_widths['amt']-2*mm, y-5*mm, f"{int(b['amt']):,}"); cur_l3_lbl = None

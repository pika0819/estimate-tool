...
# インデント
INDENT_L1 = 1.0 * mm; INDENT_L2 = 2.5 * mm; INDENT_L3 = 4.5 * mm; INDENT_ITEM = 6.0 * mm

# 表示順設定
SORT_ORDER = {
    "建築工事": ["共通仮設工事", "直接仮設工事", "特殊基礎工事", "基礎工事", "木工事", "内装材", "外壁・断熱工事", "屋根・板金工事", "屋根改修工事", "鋼製建具工事", "木製建具工事", "塗装工事", "内装工事", "左官工事", "左官・タイル工事", "金物工事", "家具工事", "設備機器工事", "制震耐震工事", "雑工事", "玄関改修工事", "台所改修工事", "和室改修工事"],
    "換気・暖房設備工事": ["換気工事"], "電気設備工事": ["配線工事", "照明工事"],
    "給排水衛生設備工事": ["屋外給水設備工事", "屋外排水工事", "屋外雨水工事", "器具・設置工事", "屋内給水配管工事", "オイルタンク移設費", "諸経費"],
    "諸経費": ["諸経費"]
}
...
def load_data_from_spreadsheet(sheet_url):
    try:
        ...
        info_dict = {str(row[0]).strip(): str(row[1]).strip() for row in data_info if len(row) >= 2 and row[0]}
...
def create_estimate_pdf(df, info_dict):
    ...
    params = {
        ...
        'expiry': info_dict.get('見積もり書有効期限', ''), 'date': info_dict.get('発行日', ''),
        'company_name': info_dict.get('会社名', ''), 'ceo': info_dict.get('代表取締役', ''),
        'address': info_dict.get('住所', ''), 'phone': info_dict.get('電話番号', ''),
        'fax': info_dict.get('FAX番号', ''), 'spec': info_dict.get('見積もり仕様', '')
    }
    ...
    def draw_page2():
        ...
        curr_y = box_top - 15*mm; label_x = box_left + 38*mm
        ...
        wareki = to_wareki(params['date'])
        c.setFont(FONT_NAME, 12); c.drawRightString(width - 40*mm, box_top + 5*mm, wareki)
        c.showPage()
    ...
    def draw_page3_total_summary(p_num):
        ...
        y_foot = bottom_margin + 21*mm
        while y > y_foot + 0.1: draw_grid_line(y - row_height); y -= row_height
        y = y_foot
        ...
        for l1 in sorted_l1:
            ...
            for i_l2, l2 in enumerate(sorted_l2):
                ...
                for b in block_items:
                    ...
                    if y - row_height < bottom_margin - 0.1 and itype != 'footer_l1':
                        temp_y = y
                    ...
                    if itype == 'header_l2': draw_bold_string(col_x['name']+INDENT_L2, y-5*mm, b['label'], 10, COLOR_L2); l2_has_started = True
                        ...
                        draw_bold_string(col_x['name']+INDENT_L2, y-5*mm, b['label'], 10, COLOR_L2); c.setFont(FONT_NAME, 10); c.setFillColor(COLOR_L2); c.drawRightString(col_x['amt']+col_widths['amt']-2*mm, y-5*mm, f"{int(b['amt']):,}")
                        c.setLineWidth(1); c.setStrokeColor(COLOR_L2); c.line(x_base, y, right_edge, y)
                    elif itype == 'footer_l1':
                        draw_bold_string(col_x['name']+INDENT_L1, y-5*mm, b['label'], 10, COLOR_L1); c.setFont(FONT_NAME, 10); c.setFillColor(COLOR_L1); c.drawRightString(col_x['amt']+col_widths['amt']-2*mm, y-5*mm, f"{int(b['amt']):,}")
    ...
    p_next = draw_page3_total_summary(1); draw_details(p_next)
...
if not st.session_state.process_done:
    ...
    if st.button("見積書を作成してURLを破棄する", type="primary"):
        if not input_url: st.error("URLを入力してください。")
        else:
            with st.spinner('データを読み込み中...'):
                df, info_dict = load_data_from_spreadsheet(input_url)
                if df is not None and info_dict is not None:
                    pdf_bytes = create_estimate_pdf(df, info_dict)
                    date_str = info_dict.get('発行日', '0000').replace('/', '').replace('-', '').replace('年', '').replace('月', '').replace('日', '')
                    st.session_state.file_name = f"{date_str}_{info_dict.get('施主名', '無名')}_{info_dict.get('工事名', '工事')}_{info_dict.get('見積もり仕様', '仕様')}.pdf"
                    st.session_state.pdf_data = pdf_bytes; st.session_state.process_done = True; st.rerun()
else:
    st.success(f"✅ 見積書の作成が完了しました。")
    col1, col2 = st.columns(2)
    with col1: st.download_button(label="📥 ダウンロード", data=st.session_state.pdf_data, file_name=st.session_state.file_name, mime="application/pdf")
    with col2:
        if st.button("別の見積もりを作成する"):
            st.session_state.process_done = False; st.session_state.pdf_data = None; st.rerun()

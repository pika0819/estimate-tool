...
def create_estimate_pdf(df, params):
    ...
    def draw_page_header_common(p_num, title_text="内 訳 明 細 書"):
        ...
        c.setFont(FONT_NAME, 10); c.drawRightString(right_edge, hy, params.get('company_name', ''))
    ...
    def draw_page1():
        ...
        draw_bold_centered_string(width/2, height - 110*mm, f"{params.get('client_name', '')}  様", 36)
        c.setLineWidth(1); c.line(width/2 - 80*mm, height - 112*mm, width/2 + 80*mm, height - 112*mm)
        draw_bold_centered_string(width/2, height - 140*mm, f"{params.get('project_name', '')}", 24)
        c.setLineWidth(0.5); c.line(width/2 - 80*mm, height - 142*mm, width/2 + 80*mm, height - 142*mm)
        wareki = to_wareki(params.get('date', ''))
        c.setFont(FONT_NAME, 14); c.drawString(40*mm, 50*mm, wareki)
        x_co = width - 100*mm; y_co = 50*mm
        draw_bold_string(x_co, y_co, params.get('company_name', ''), 18)
        c.setFont(FONT_NAME, 13); c.drawString(x_co, y_co - 10*mm, f"代表取締役   {params.get('ceo', '')}")
        c.setFont(FONT_NAME, 11); c.drawString(x_co, y_co - 20*mm, f"〒 {params.get('address', '')}")
        c.drawString(x_co, y_co - 26*mm, f"TEL: {params.get('phone', '')}")
    ...
    def draw_page2():
        ...
        c.setFont(FONT_NAME, 20); c.drawString(40*mm, height - 50*mm, f"{params.get('client_name', '')}  様")
        ...
        c.setLineWidth(0.5); c.rect(box_left+1.5*mm, box_top - box_height + 1.5*mm, box_width-3*mm, box_height-3*mm)
        label_end_x = box_left + 38*mm; colon_x = label_end_x + 1*mm; val_start_x = colon_x + 5*mm
        line_sx = box_left + 10*mm; line_ex = box_left + box_width - 10*mm; curr_y = box_top - 15*mm
        c.setFont(FONT_NAME, 14); c.drawRightString(label_end_x, curr_y, "見積金額"); draw_bold_string(colon_x, curr_y, "：", 14)
        ...
        c.setLineWidth(0.5); c.line(line_sx, curr_y-2*mm, line_ex, curr_y-2*mm); curr_y -= 18*mm
        items = [("工 事 名", params.get('project_name','')), ("工事場所", params.get('location','')), ("工   期", params.get('term','')), ("そ の 他", "別紙内訳書による"), ("見積有効期限", params.get('expiry',''))]
        for label, val in items:
            c.setFont(FONT_NAME, 12); c.drawRightString(label_end_x, curr_y, label); c.drawString(colon_x, curr_y, "：")
            c.setFont(FONT_NAME, 13); c.drawString(val_start_x, curr_y, val); c.line(line_sx, curr_y-2*mm, line_ex, curr_y-2*mm); curr_y -= 12*mm
        x_co = box_left + box_width - 90*mm; y_co = box_top - box_height + 10*mm
        c.setFont(FONT_NAME, 13); c.drawString(x_co, y_co + 15*mm, params.get('company_name',''))
        c.setFont(FONT_NAME, 11); c.drawString(x_co, y_co + 10*mm, f"代表取締役   {params.get('ceo','')}")
        c.setFont(FONT_NAME, 10); c.drawString(x_co, y_co + 5*mm, f"〒 {params.get('address','')}")
        c.drawString(x_co, y_co, f"TEL {params.get('phone','')}  FAX {params.get('fax','')}")
        wareki = to_wareki(params.get('date', '')); c.setFont(FONT_NAME, 12); c.drawString(width - 80*mm, box_top + 5*mm, wareki)
        c.showPage()
    ...
    def draw_page4_breakdown(p_num):
        ...
        for l1_name in sorted_l1:
            data = breakdown_data[l1_name]; l2_items = data['items']; sorted_l2 = sorted(l2_items.keys(), key=lambda k: SORT_ORDER.get(l1_name, []).index(k) if k in SORT_ORDER.get(l1_name, []) else 999)
            if y - (len(sorted_l2) + 2) * row_height < bottom_margin:
            ...
            c.setFont(FONT_NAME, 10); c.setFillColor(COLOR_L1); c.drawRightString(col_x['amt']+col_widths['amt']-2*mm, y-5*mm, f"{int(data['total']):,}")
            draw_grid_line(y - row_height); y -= row_height
        while y > bottom_margin + 0.1: draw_grid_line(y - row_height); y -= row_height
    ...
    def draw_details(start_p_num):
            ...
            for i_l2, l2 in enumerate(sorted_l2):
                ...
                for b in block_items:
                    ...
                    if y - row_height < bottom_margin - 0.1 and not force_stay:
                        draw_vertical_lines(y_start, bottom_margin) # ★改ページ前に縦線を引く
                        c.showPage(); p_num += 1; draw_page_header_common(p_num, "内 訳 明 細 書 (詳細)"); y = y_start
        ...
        while y > bottom_margin + 0.1: draw_grid_line(y - row_height); y -= row_height
        draw_vertical_lines(y_start, y) # ★ページ末尾で現在のy位置まで縦線を引く
...
if not st.session_state.pdf_ready:
    with st.sidebar:
        st.header("🔑 セキュリティ入力")
        # ★方法②：type="password"削除、label_visibility="collapsed"
        input_url = st.text_input("スプレッドシートURL", label_visibility="collapsed", placeholder="スプレッドシートURLを貼り付け")
        ...
        if not input_url:
            ...
            with st.spinner('データを読み込み中...'):
                ...
                if df is not None and info_dict is not None:
                    # 日付処理
                    raw_date = info_dict.get('発行日', datetime.today().strftime('%Y%m%d'))
                    clean_date = raw_date.replace('/','').replace('-','').replace('年','').replace('月','').replace('日','')
                    filename = f"{clean_date}_{info_dict.get('施主名','無名')}_{info_dict.get('工事名','工事')}_{info_dict.get('見積もり仕様','見積')}.pdf"
